...
However, the effect of missing data on topological recovery varies with different methods to infer topology and different metrics to compare topologies.
%Results 2
%   -Additionally, parameters have different effect
Additionally, within a method and a metric, the parameters used to remove data have different effect on the ability to recover the correct topology.
%Wrap up results
%   -Thus, Baycon is better with loads of living
Thus, we found that using the topologies of Bayesian consensus trees performed consistently better than other methods for recovering the correct topology (Fig.~\ref{Fig_Results-global_perparam} and Fig.~\ref{Fig_Results-paircomp_within}) and that both the amount of available characters ($M_{C}$) and the amount of data for the living taxa ($M_{L}$) are crucial missing data parameters.

%§2
%Results from litterature
%   -Missing data per se is not a problem, problem is in the amount of overlap
As proposed in numerous previous studies, when recovering topology, the amount of missing data \textit{per se} is not a problem as long as enough data overlaps in the matrix \citep[e.g.][]{rouresite-specific2011,pattinsonphylogeny2014}.
%Proposed hypothesis
%   -One can increase overlap by adding living taxa
To improve topological recovery in a Total Evidence framework, one can efficiently increase the amount of overlapping data by sampling a maximum of characters and by increasing the number of living data coded in the morphological part of the matrix.
...
\begin{enumerate}
\item{$M_{L}$:}
One should put more effort in using natural museums history collections for coding the missing morphological data for living taxa if possible
\item{$M_{F}$:}
the amount of missing data unfortunately depends on the quality of the fossil record and can not be actively improved and depends on exceptional discoveries \citep[e.g.][]{nithe2013}
\item{$M_{C}$:}
improvement can be done by vast collaborative projects in order to gather as much characters as possible \citep[e.g.][]{O'Leary08022013}. Therefore, we advocate the importance of coding morphological characters for the most living taxa possible and with the most characters possible, potentially by using collaborative projects portals such as morphobank \citep{morphobank}.
\end{enumerate}
...
Thanks to Fr\'{e}d\'{e}ric Delsuc, Emmanuel Douzery, Trevor Hodkinson and Andrew Jackson, Gavin Thomas, April Wright and the members of the Macro Journal Club %TG : also Adam, Kev and Sive or that's putting to many people, thinking about it, they helped at least as much as Andrew (not mentioning Trevor but that's more political)?
