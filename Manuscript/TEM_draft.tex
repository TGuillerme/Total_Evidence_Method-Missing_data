...
We used the Triples \citep{dobson1975triplets} metric to assess the number of conserved taxa across trees, and the Robinson-Fould \citep{RF1981} distance to identify conserved clade positions.
...
The triples distance ($T_{x,y}$; \citealp{dobson1975triplets}) measures the number of sub-trees made up of three taxa that differ between two given trees (\citealt{critchlowthe1996} ; see \hyperref[SupplementaryMaterial]{Supplementary Material S2}).
...
% NC: path difference or *distance*? - TG: difference.
    , measures the number of shared clades across two trees. The metric reflects the distance between the distributions of tips among clades in the two trees (\citealt{RF1981} ; see \hyperref[SupplementaryMaterial]{Supplementary Material S2}).
...
%TG: maybe draw a figure to illustrate that? Triples = not really stric, RF= really strict.
...
Therefore a low value indicates that clades are similar across the two trees, whereas a high value indicates that there are few common clades (see \hyperref[supplementaries]{supplementaries}). %NC: See supp for what purpose exactly?
...
For any $m,n$, when \textit{NTS} = 1, the trees are identical, when \textit{NTS} = 0 the trees are no more different than expected by chance, and when \textit{NTS} $<$ 0, the trees are more different than expected by chance %chance.\\
...
Thanks to Trevor Hodkinson and Andrew Jackson %Also Gavin Thomas, Emmanuel Douzery and Frédéric Delsuc if not authors. %NC: Also members of MacroJC - %TG : also Adam Kev and Sive or that's putting to many people, thinking about it, they helped at least as much as Andrew (not mentioning Trevor but that's more political)?
