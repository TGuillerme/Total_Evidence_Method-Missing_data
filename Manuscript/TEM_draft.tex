...
-Overall missing data is doesn't affect topological recovery in Bayesian (around minimum 70\% of topological topology in the worth scenario).
...
In our Maximum Likelihood analyses, topological recovery (i.e. tree similarity) decreased as the percentage of missing data increased, using both the Robinson-Foulds and Triplets metrics.
...
%For the Triplets metric there is no significant effect of the $M_L$ parameter, in other words, if living taxa are removed, there is no effect on the placement of "flying" taxa.
%For the Robinson-Foulds metric, there is a significant effect of the $M_L$ leading small decrease in topological recovery when $M_L$ increases (see Table ~\ref{Ba_RF-ML_results}).
...
In Maximum Likelihood framework, the effect of missing data in the fossil record appeared to be constant and led to a constant decrease in topological recovery when the missing data increase.
...
Interestingly, 10\% or 25\% missing data for the $M_F$ parameter does not seems to affect the apparition of unstable taxa .
...
There is no significant effect of the $M_F$ parameter when using the Triplet metric but the effect is significant when using the Robinson-Foulds metric .
...
The number of missing morphological characters ($M_C$), however, seems to be more affecting topological recovery than $M_L$ and $M_F$ .
The Robinson-Foulds metric shows a rapid drop in tree recovery from 10\% of missing data (Robinson-Foulds NTS mode $<$ to 0.5 from 10\% missing data .
This decrease is however slower when using the Triplets metric with still a good topological recovery at 10\%  (Triplets NTS modes = 0.95 .

The effect of $M_C$ in Bayesian framework is similar as the one described for $M_L$ and $M_F$: tree recovery is only slightly better than expected by chance and only the Robinson-Foulds metric shows a significant effect of the $M_C$ parameter on tree recovery.
...
%The decrease in clade conservation (low Robinson-Foulds NTS score) is faster than the increase in flying taxa (low Triplets NTS score).
...
% NC: I would make these tables wider - so have a column for Triplets, and one for RF, rather than a column for metric. same with Bayesian vs ML, why not have a set of columns instead? It'll take up less space and be easier to read I think...
