...
Although branch length estimation is also important (namely for timing extinction and/or speciation events - e.g. \citealt{ronquista2012}), we do not consider branch lengths in this study. This is partially due to difficulties of estimating the effect of missing data simultaneously on branch length and topology, % TG: or "This is partially due to difficulties with simulating branch lengths and topology simultaneously"
...
\subsubsection{Paired trees comparisons} % NC: OK I'm lost in this section! We should discuss then think about how to express this in an understandable fashion. TG: or no subheading at all? See next section as well.
...
%   A.1-Missing data reduce topological recovery regardless the method, etc... -> figure topological recovery ~ parameters(all - no interactions) + methods(all) + metrics(all)
%   A.2-Within a particular method, missing data reduce topological recovery   -> figure topological recovery ~ parameters(all + interaction) + methods(ML/baycon) + metrics(all)
% B-"Significance" (i.e. Bhattacharrya Coefficients as a two-sided t-test. Or in other words, does the distributions between two stuff (within/among) differ, regardeless who's better or not:
%   B.1-Effect of the parameters  -> figure matrix
%   B.2-Effect if the method      -> table matrix 
...
% B.2 - among methods
%The Results for the Bayesian consensus tree in RF are the more dissimilar to the results of the Maximum Likelihood bootstrap trees and to the Bayesian posterior distribution trees.
%The Maximum Likelihood bootstrap trees and the Bayesian posterior distribution trees are the most similar for both metrics.
