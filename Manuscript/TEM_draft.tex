...
\end{center}
\medskip
\noindent{\bf Corresponding author:} \textit{Thomas Guillerme, School of Natural Sciences, Trinity College Dublin, Dublin 2, Ireland; E-mail: guillert@tcd.ie; Fax: +353 1 6778094; Tel: +353 1 896 2571.}\\
\vspace{1in}
...
We find that increasing the number of living taxa with morphological characters (1) and the overall amount of morphological data (3) have a greater impact on the ability to recover the "correct" topology than increasing the data completeness in the fossil record (2). Therefore, we suggest that sampling effort should be focusing on these two first aspects to increase the accuracy of topology in the Total Evidence framework. Additionally, we found that the Bayesian consensus tree consistently recovers topology that are closest to the "correct" topology than any other method. Therefore, we also suggest, after the data collection effort, that one should fix the tree topology using the Bayesian consensus tree topology for any further tree inference or macroevolutionary analysis.
