...
%Gavin's general comment:I have made some comments on Thomas’ paper. I think it generally looks good. The one thing that I think might be tricky to get past reviewers is the comparison between the ML and Bayesian trees. The problem is that you can’t tell if the poorer performance of ML is because of the inference method or the difference between the substitution models - the ML uses an overparameterised model (GTR) compared to the generating model, whereas the Bayesian analyses do not. It might be possible to cite your way out of this but I wouldn’t be surprised if a reviewer asks for new analyses by either using different ML software that can handle the HKY model (e.g. Garli) or running the Bayesian with the GTR model (although that might not be adequate because if I remember correctly RAXML uses an approximation for gamma that is not the same as in other software).
...
% NC: I think you misunderstood Nick's comment here. He meant that living > fossil but that you can't put the number of characters in here too. I've reverted back to the original as I'm not sure his solution is correct
Therefore, we suggest that sampling effort should be focused on morphological data collection for living species to increase the accuracy of topological inference in a Total Evidence framework. Additionally, we find that Bayesian methods consistently outperform other tree inference methods. We therefore recommend using Bayesian consensus trees to fix the tree topology prior to further analyses.

%Hummm Nick's comment again - That's a point to make in the discussion maybe. But let's wait for Gavin comments...
%it's possible that adding dating information etc. in tip-dating makes Bayesian dated total-evidence analyses (ie tip-dating) better than non-dated Bayesian total-evidence analyses (typical total-evidence approach), because more information is being used, and because dates give some information about phylogenetic position.  In any case, the study wasn't about tip-dating
% NC: Simplest thing here is to just leave out the mention of tip dating. Will need to fix in discussion
% NC: Also Nick makes another sensible comment about *why* are living species more important. Check this is covered in discussion 
%TG: I think that's covered in \subsection{Individual effects of missing data parameters} - \subsubsection{Missing data for living taxa ($M_{L}$)}. (If no living taxa, the fossils cannot branch.)
...
%G: Why does it need so much data? Is there any evidence (a citation or two) to back this up? TG: added clarity in both sentences below.
...
%G: Might be worthing reading and citing this: http://www.sciencemag.org/content/333/6041/448.full. Missing data can be a problem if it renders some subtrees unresolvable. TG: added citation in the whole bunch.
% NC: Did you actually *read* this paper? It makes the point that it *is* a problem if it affects certain subtrees. Might need to add a small caveat into the bit in brackets above... TG: Ok, so this comment goes with the data overlap. I’m not entirely sure where the paper comes in. If the there is not enough overlap in the super matrix, then it is impossible to get a consensus clade tree (kind of supporter talked in the paper) per terrace?
...
%G: Alan Lemmon has done a fair amount of work on missingness - worth checking out the work from his lab, e.g. http://www.evotutor.org/LemmonLab/Pdfs/Lemmon-etal2009.pdf TG: added reference (that was actually in the 2014 draft)
% NC: OK. But be careful here. When people give you comments you want to take them seriously and actually read the papers they suggest rather than just adding references. Are you sure Lemmon is just looking at molecular stuff? Think Gavin's point might be that he's done stuff on missingness in morphological data too which at present we don't really cite or cover at all. TG: I don’t like arguing on comments but this paper is about missing data in molecular matrices (still relevant though). The reason I removed it from the old version is that it states something kind of similar than all the others: overlap but it is specially important in bigger matrices where missing data can weaken the support in certain parts of the tree. The idea being probably close to the tree terrace paper.
...
% NC: The next few sentences could be moved to the methods. They feel a bit clunky here. TG: I like the way they end the intro: "we're simulating stuff but it links to more real stuff in reality."
% NC: OK but hey currently don't make sense in terms of what was said perviosuly so I've numbered them. Still don't like this. It really should be integrated with the parameters. Basically it's poorly structured and doesn't follow very well - that's what I mean by "clunky"  - TG: You're right (as usual ;)) it works better without.
%These parameters reflect empirical biases in data availability as follows: (i) the increased availability of molecular data means that morphological data for living species is rarely collected, and few people have the skills to identify characters needed for detailed phylogenetic analysis; (ii) missing data in fossil taxa is common due to preservation biases \citep{sansomfossilization2013}; and (iii) the overall number of characters depends on the effort of the people identifying them \citep[e.g.][]{O'Leary08022013}.
...
%TG: I swapped the results to be consistent, ML and MC first and then Bayesian
...
%G:I don’t think there is a problem with this split but I expect reviewers might ask for justification. Is this typical of existing TE studies? TG: avoiding bias comment.
% NC: I think you misunderstood Gavin's point. He knows why you did it like this, but if usually TE studies are 95% living and 5% fossil (for example) then your results aren't really viable in real world. He ideally wants a statement like: this is a typical split between fossil and living tax across empirical TE studies (cite papers). However, I doubt this is the case. Actually aren't TE studeis usually dominated by fossil taxa?
% TG: Really depends. In the 5 I keep citing, Pyron is close to 50/50, Ronquist and Slater are more towards living and Beck is more towards fossil
Empirical Total Evidence matrices can be biased towards a greater number of fossils (e.g. 88\% and 58\% fossils taxa in respectively \citealt{beckancient2014} and \citealt{schragocombining2013}) or towards more living taxa (e.g. 69\% of living taxa in both \citealt{ronquista2012} and \citealt{slaterphylogenetic2013}) buy they can contain an even number of both type of taxa (e.g. 52\% of living taxa in \citealt{pyrondivergence2011}). Therefore, to avoid biasing our simulations towards the two extreme situations (more living or fossil taxa) and to make each simulation comparable, we implemented a rejection sampling algorithm to select only trees with 25 living and 25 fossil taxa.
...
%TG: this part was removed from the into: These parameters reflect empirical biases in data availability as follows: (i) the increased availability of molecular data means that morphological data for living species is rarely collected, and few people have the skills to identify characters needed for detailed phylogenetic analysis; (ii) missing data in fossil taxa is common due to preservation biases \citep{sansomfossilization2013}; and (iii) the overall number of characters depends on the effort of the people identifying them \citep[e.g.][]{O'Leary08022013}.

\begin{enumerate}
\item{$M_{L}$, the proportion of living taxa with no morphological data: 0\%, 10\%, 25\%, 50\% or 75\%.}
This parameter illustrates the number of living taxa that are present in the molecular part of the matrix but not in the morphological part. This reflects the fact that morphological data for living species are scarce. This can be due to the increased availability of molecular data and/or that few people have the skills to identify characters needed for detailed phylogenetic analysis.
...
%G: I think you need to justify using the GTR model (and so also RAXML). The data were simulated under an HKY model so you are fitting an overparameterised model. It might be that the better results for the MrBayes analyses come down to model misspecification.  You can certainly say that HKY is not an option in RAXML but then you might be asked why you don’t use another program, e.g Garli.
...
% NC: This isn't really a justification. Also by pointing out the problem you're drawing attention to it. I actually never noticed this when reading the previous drafts. But I've never used RAxML.

To measure the support for each branch in our simulated phylogenies
%G: Do you mean you tested whether the simulated data generally had any phylogenetic signal with which to recover a tree? I was a bit unsure but mainly because I am hardwired to thinks of Pagel’s lambda when anyone mentions phylogenetic signal…
% NC: Haha that is exactly what I've said about a million times to you.
% TG: And after the literature review for the next paper (missing living in mammals) on community structure & Co. I start to get where you guys are comming for ;).
...
%G: Why these two metrics? Might need a few sentences to justify them in addition to the explanation of what they are that you already have.
%We used these metrics because they are widely used in literature \citep[][e.g.]{Bogdanowicz2012}, their relative performance is well described \citep{Kuhner04112014} % NC: Is that what you mean. "they relative performance" means nothing to me. 
%and because they illustrate two different aspects of tree topology (see Discussion). TG: ok I've fixed that towards: "as said above, they describe two different things but also they're cool because we know how they behave and the code is available to calculate them." - a bit more honest.
...
%G: This might be expected given the GTR rather than HKY model is used. 
 trees than for the Bayesian consensus trees (median Bhattacharrya Coefficient = 0.69, 0.48 and 0.66 for $M_{L}$, $M_{F}$ and $M_{C}$ respectively; Fig.~\ref{Fig_Results-permeth_perparam}; online Appendix Tables %~\ref{Tab_Supp_summary_BC_ML}, ~\ref{Tab_Supp_summary_BC_MF} and ~\ref{Tab_Supp_summary_BC_MC}
S5, S6 and S7 %Modified for build without supplementaries
). 
However, The Normalised Triplets distance is similar between the Maximum Likelihood trees and the Bayesian consensus trees for all the parameters ($M_{L}$, $M_{F}$ and $M_{C}$) (median Bhattacharrya Coefficient = 0.84, 0.75 and 0.80 for $M_{L}$, $M_{F}$ and $M_{C}$ respectively; Fig.~\ref{Fig_Results-permeth_perparam}; online Appendix Tables %~\ref{Tab_Supp_summary_BC_ML}, ~\ref{Tab_Supp_summary_BC_MF} and ~\ref{Tab_Supp_summary_BC_MC}
S5, S6 and S7 %Modified for build without supplementaries
).

\subsection{Combined effect of missing data parameters}
As expected, our ability to recover the "best" tree topology is worst when each parameter contains the maximum amount of missing data (i.e. $M_{L}$ = 75\%, $M_{F}$ = 75\% and $M_{C}$ = 75\%), and best when there is no missing data (i.e. $M_{L}$ = 0\%, $M_{F}$ = 0\%, $M_{C}$ = 0\%; Fig.~\ref{Fig_Results-global_perparam}; online Appendix Tables %~\ref{Tab_Supp_summary_metric_ML}, ~\ref{Tab_Supp_summary_metric_MF} and ~\ref{Tab_Supp_summary_metric_MC}
S2, S3 and S4 %Modified for build without supplementaries
...
%G: How does tree resolution influence the tree comparison metrics? Are the Bayesian trees always fully resolved? Could that bias towards shorter distances to the best tree (because there is reduced probability of conflict). TG: This comment is kind of linking to the following.

% NC: Yes BUT the point is that Gavin misunderstood so you need to be clearer! Add a comment earlier that points out that the Bayesian consensus trees are not fully resolved (if this is true). If they are fully resolved more often than the ML trees you may need t oadd a paragraph to the discussion as Gavin is right, this could explain the result. TG: this is now explained in the following paragraph after this one mentioning in details the polytomy stuff. Or should be coming before?
...
Also, because the Bayesian consensus tree are more likely to contain polytomies when some branching are poorly resolved (in $<$ 50\% of the trees) in the "best" tree, the Bayesian consensus "missing-data" trees have a higher probability to be less distant to the "best" tree than in a Maximum Likelihood framework. However we minimized this effect by only selecting matrices that had a median bootstrap for all their nodes $>$ 50 (see Methods, section 3).
...
% NC: OK *none* of this is a justification. More the opposite! We know GTR is a generalisation of HKY, that doesn't make it better. That's basically what we mean when we say the ML model is overparameterised because even if all parameters = 1 it's a HKY but with fewer df. Thus over parameterised! Then yes, this would be a bigger issue for Bayesian - but so what? It's not an issue in Bayesian here. It's not like you had to choose to do it for one or the other, thus ML was a better choice. So this isn't a justification, it's just a series of factual statements.
...
% NC: Huh??? Not really following, nor is this a good argument.
...
%^Nick
%it's possible that adding dating information etc. in tip-dating makes Bayesian dated total-evidence analyses (ie tip-dating) better than non-dated Bayesian total-evidence analyses (typical total-evidence approach), because more information is being used, and because dates give some information about phylogenetic position.  In any case, the study wasn't about tip-dating
% NC: Note that Nick's point is that we can really only say what works best for topology only methods
...
% NC: Why would it improve spatial methods? I've removed this.
...
\begin{table}[ht]
\caption{
%G: I think this table needs to be decoded a bit. I wasn’t really sure what it was supposed to be getting across. Maybe it is because I am not familiar with the Bhatt… distance but I suspect that many readers will be similarly unfamiliar. TG: fixed clarity
Bhattacharyya Coefficients of the pairwise method comparisons, each of which corresponds to the normalised distance between the "best" tree and the "missing data" trees using either the Normalised Robinson-Foulds distance (RF) or the Normalised Triplets distance (Tr). Each line summarizes the distribution of the probability of overlap between pairs of tree inference methods. The values highlighted in bold are the extreme values of high or low probability of overlap between two methods. If two methods have a high probability of overlap, they have a similar ability to recover the "correct" tree topology. % NC: check this is right, I think that's what Gavin wanted to be added - a way of interpreting what is going on
}
\centering
...
\label{Fig_Bhattacharyya_Coefficients1} %Bhattacharyya coefficients calculation outline
\end{figure}
...
\label{Fig_Bhattacharyya_Coefficients2} %Bhattacharyya coefficients calculation outline
\end{figure}

% NC: These are still really hard to follow but I'm at a loss as to how to explain them better.
% TG: will the t-test analogy be useful here?
...
%G: Is there a scale to the colour or is it binary? Also, I don’t get the axes. Am I missing something or are they the same? TG: fixed clarity.
}
