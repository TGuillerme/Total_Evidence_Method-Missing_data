...
\subsection{Generating the matrix} % NC: Can you number these subsections like they are in the steps above?
...
Using these parameters took $~$8 % NC: again tilde isn't appearing on my PDF 
...
% NC: Do you mean 50 x 10^6? or 5 x 10^7? It's standard to use 10^x not 1^x
...
We used these priors to speed up the Bayesian estimation process. These priors biased the way the Bayesian process calculated branch lengths by giving non-random starting points and boundaries for parameter estimation however, here we are focusing on the effect of missing data on tree topology and not branch lengths. Even using these priors, it took $~$ % NC: Another tilde issue
...
% NC: You may have a better definition of wildcard taxa than mine :)
...
% NC: Why? For more details? Simulations etc
...
% NC: If I understand right, you use RF normalised and TD normalised. NOT RF, TD and NTS. So having these as three separate sections seems odd. Instead I'd write RF section, then explain that it is not comparable across trees with different numbers of taxa so you normalise it as suggested in Bogdanowicz's NTS method. Then describe TD and explain that you normalise it as discussed above for RF. NTS isn't really a metric, just a simple normalisation procedure.

\subsubsection{Tree comparisons} % NC: OK I'm lost in this section! We should discuss then think about how to express this in an understandable fashion.
\label{tree_comparisons}
For the Maximum Likelihood and Bayesian consensus
% NC: is this actually a maximum clade credibility tree
...
% NC: Yeah probably. You could also fit this into the section above. Again NTS isn't really a metric in my opinion...
...
To take into account the uncertainty of tree inference in both Maximum Likelihood and Bayesian trees, % NC: instead of using the single ML or B MCC tree as above,
we extracted 1000 random bootstrapped trees from the Maximum Likelihood analysis and 1000 from the posterior tree distribution of the Bayesian analysis. % for the best trees and then did the same for the 125 missing data trees (resulting in 125 x 1000 missing data trees). 
...
We repeated all of the above for each of the 50 chains. We then calculated the mode and the 50\% and 95\% confidence intervals from the resulting distribution using the hdrcde R package v. 3.1 \citep{hdrcde}. %Finally we tested for significant differences among the different distributions of trees using non-parametric group and pairwise comparisons by using respectively Kruskal-Wallis and Nemenyi-Damico-Wolfe-Dunn test (CITE R PACKAGE)\citet{ruxtontime2008}. TG: finally we didn't (Andrew's advice - see Battacharyya)

% NC: New subheading (summarising)

% NC: Finally after doing all theses stupid comparisons and simulations we tested the effects of our missing data parameters (ML) on our ability to recover the correct topology. 
To do this we measured the similarity among the different distributions % NC: OF WHAT!!! tree metrics for all of the analyses above
using the Bhattacharyya Coefficient \citep{Bhattacharyya}. The Bhattacharyya Coefficient is the probability of overlap between two distributions, ranging from 0 to 1 (\citealt{Bhattacharyya} - see \hyperref[SupplementaryMaterial]{Supplementary Material Section 2} % FOR WHAT!!!). 

%For each method (Maximum Likelihood trees, Bayesian %MCC?
%trees, Bootstraps and Bayesian posterior trees) and metric (Robinson-Foulds and Triplets distance), we used this coefficient in two ways in order to:
\begin{enumerate}
...
% Add in your three questions here?
%---------------------------------------------
%
%       RESULTS
%
