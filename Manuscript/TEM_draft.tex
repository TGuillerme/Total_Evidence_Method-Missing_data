...
% NC: You could add all the packages etc to a template .sty file to save writing it all up here. Rich's ms.sty that I provided on DropBox is a good example - TG: couldn't find that one...
...
%TG: how about this:
The morphological data for living taxa is rarely collected when molecular data is available (e.g. \citet{O'Leary08022013} vs. \citet{meredithimpacts2011}), and for fossil taxa, the scarcity of the fossil record only allow to collect the data available (for example, in vertebrates, the solidest parts of the skeleton \citet{sansomfossilization2013}).
...
In our Maximum Likelihood analyses, topological recovery (i.e. tree similarity) decreased as the percentage of missing data increased, using both the Triples and Robinson-Foulds metrics (see Table ~\ref{Group_results} - Fig.~\ref{Fig_Results}). However, topological recovery was not significantly different for trees with 25\%, 50\% and 75\% missing morphological data for living taxa (Fig 1b???), % is this what you meant? And indicate the particular panel of the figure it refers to
...
\begin{table}
  \centering
  \linespread{1.0}
  \caption{$M_F$ non-parametric pairwise difference for Robinson-Foulds metric in ML framework} %10/06 - Missing 8 ML Chains
  \input{Tables/ML_RF-MF_results}
...
Thanks to Trevor Hodkinson and Andrew Jackson %Also Gavin Thomas, Emmanuel Douzery and Frédéric Delsuc if not authors. %NC: Also members of MacroJC
for useful comments on our simulation protocol and Paddy Doyle for  assistance with the computer cluster. Simulations used the Lonsdale cluster maintained by the Trinity Centre for High Performance Computing and funded through grants from Science Foundation Ireland. % NC: Is this what you're supposed to cite for the cluster? Check.
