...
%V1
Although the Total Evidence approach is very promising, it requires many data and is therefore likely to suffer from missing data issues that may affect its ability to infer correct phylogenies.
%V2
Although the Total Evidence approach is very promising, morphological characters are often limited and missing, therefore this method is likely to suffer from missing data issues that may affect its ability to infer correct phylogenies.

Here we use simulations to assess the effects of missing data on tree topologies inferred from Total Evidence matrices. We investigate three major factors that directly affect the completeness 
%How does the overall number of characters affect completeness of the matrix?
and the size
of the morphological part of the matrix: the proportion of living taxa with no morphological data, the amount of missing data in the fossil record, and the overall number of morphological characters in the matrix. We infer phylogenies from complete matrices and from matrices with various amounts of missing data, and then compare missing data topologies to the "best" tree topology inferred using the complete matrix.
...
%G: Might be worthing reading and citing this: http://www.sciencemag.org/content/333/6041/448.full. Missing data can be a problem if it renders some subtrees unresolvable.
...
Here we use a simulation approach to assess the effect of missing data on tree topologies inferred from Total Evidence matrices. Since the molecular part of a Total Evidence matrix acts like a "classical" molecular matrix containing only the living taxa \citep{ronquista2012}, the effect of missing data on such matrices is well known \citep{wiensmissing2006,wiensmissing2008,rouresite-specific2011}.
%G: Alan Lemmon has done a fair amount of work on missingness - worth checking out the work from his lab, e.g. http://www.evotutor.org/LemmonLab/Pdfs/Lemmon-etal2009.pdf
...
%G:I don’t think there is a problem with this split but I expect reviewers might ask for justification. Is this typical of existing TE studies?
...
The "best" tree and the "missing-data" trees were inferred using RAxML v. 8.0.20 \citep{Stamatakis21012014}. For the molecular data, we used the GTR
%G: I think you need to justify using the GTR model (and so also RAXML). The data were simulated under an HKY model so you are fitting an overparameterised model. It might be that the better results for the MrBayes analyses come down to model misspecification.  You can certainly say that HKY is not an option in RAXML but then you might be asked why you don’t use another program, e.g Garli.
 + $\Gamma_4$ model (\citealt{tavare1986}; default GTRGAMMA in RAxML v. 8.0.20; \citealt{Stamatakis21012014}). For the morphological data, we used the implemented Markov \textit{k} state model \citep{lewisa2001} assuming an equal state frequency and a unique overall substitution rate ($\mu$) following a gamma distribution of the rate variation with four distinct categories (M\textit{k} + $\Gamma_4$; -K MK option in RAxML v. 8.0.20; \citealt{Stamatakis21012014}).

To measure the phylogenetic signal
...
We compared the topology of the "missing-data" trees to the "best" tree to measure the effect of the three parameters $M_{L}$, $M_{F}$ and $M_{C}$ on tree topology. We used the Robinson-Foulds distance \citep{RF1981} 
...
to assess the amount of conserved clade positions and the Triplets distance \citep{dobson1975triplets} to assess the amount of wildcard taxa  (i.e. taxa that frequently change position in different trees; \citealt{kearneyfragmentary2002}). We normalised both metrics using methods described in \citet{Bogdanowicz2012} to generalize our results for any \textit{n} number of taxa. These metrics are described in detail below.
...
\begin{table}[ht]
\caption{
%G: I think this table needs to be decoded a bit. I wasn’t really sure what it was supposed to be getting across. Maybe it is because I am not familiar with the Bhatt… distance but I suspect that many readers will be similarly unfamiliar.
...
\begin{figure}
\caption{The effects of increasing missing data 
%G: How does tree resolution influence the tree comparison metrics? Are the Bayesian trees always fully resolved? Could that bias towards shorter distances to the best tree (because there is reduced probability of conflict).
on topological recovery using Maximum Likelihood trees (black) and Bayesian consensus trees (grey). The x axis shows the percentage of missing data from 0\% (white) to 75\% (black) for the three parameters: $M_{L}$ (upper line), $M_{F}$ (middle line) and $M_{C}$ (lower line). Topological recovery was measured using two different tree comparison metrics: Normalised Robinson-Foulds distance (upper row) and Normalised Triplets distance (lower row). The graph shows the modal value (points), and the 50\% (thick solid lines) and 95\% (thin dashed lines) confidence intervals of the distributions of the tree comparison metric for each missing data parameter and tree inference method.} 
...
%G: Is there a scale to the colour or is it binary? Also, I don’t get the axes. Am I missing something or are they the same?
}
