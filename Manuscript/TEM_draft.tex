...
Our results show that the ability to recover the correct topology in a Total Evidence framework decreases with increasing missing data, regardless of how data were removed or the method of tree inference used. 
% NC: Here I've removed the tree metrics as these are just different ways of measuring topology.
%Results 1
%   -However, parameters have different effect
However, these factors affected topological recovery in different ways and to different extents. 
%Results 2
...
% NC: This is implicit in the line above
%Wrap up results
%   -Thus, Baycon is better with loads of living 
% NC: Make sure it's parameters then tree building methods.
Increasing the number of characters in the matrix without data ($M_{C}$) and the amount of missing morphological data in the matrix for living taxa ($M_{L}$) had worst effects on topological recovery (Fig.~\ref{Fig_Results-paircomp_within}). Additionally, using Bayesian consensus trees recovered the correct topology more consistently than using Maximum Likelihood trees or Bayesian posterior distributions of trees (Fig.~\ref{Fig_Results-global_perparam} and Fig.~\ref{Fig_Results-paircomp_within}).
% NC: Probably needs another summing up line here. Or maybe not

%NC: Make sure the order of the parameters is always the same L,F, C or whatever! THROUGHOUT the paper. Also correct RF and T to standardised OR normalised RF and T.
...
As proposed in previous studies, our results show that the amount of missing data is not a problem \textit{per se} for Total Evidence methods, as long as enough data overlaps in the matrix \citep[e.g.][]{kearneyfragmentary2002,wiensmissing2003,rouresite-specific2011,pattinsonphylogeny2014}. % NC: What do you mean by data overlap here? That species have overlapping characters?
%Proposed hypothesis for result 1
%   -One can increase overlap by adding living taxa
To improve the ability to recover the "correct" topology in a Total Evidence framework, one can efficiently increase the amount of overlapping data 
% NC: Again what do you mean by overlapping data? And isn't this obvious???
by reducing the missing data in each of our three missing data parameters ($M_{L}$, $M_{C}$ and $M_{F}$). In our simulations, the overlap in data is greatly reduced when all three missing data parameters have a lot of missing data (e.g. $M_{L}$ = 75\%, $M_{F}$ = 75\% and $M_{C}$ = 75\%; Fig.~\ref{Fig_Results-permeth_perparam} and Fig.~\ref{Fig_Results-global_perparam}). In this scenario, the overall number of missing characters ($M_{C}$) affects the proportion of overlapping data by reducing the overall size of the matrix (Fig.~\ref{Fig_Results-global_perparam}). % NC: What's the relevance of that sentence?
...
%§3
%Discussing results 1 % NC: This can join to the end of the previous paragraph
This variable effect of our missing data parameters on the ability to recover the "correct" topology (Fig.~\ref{Fig_Results-paircomp_within}) can be explained by % the way our parameters are illustrating the sources of missing data in empirical matrices 
% NC: No. This shows how our parameters relate to the real world and give real world solutions. Probably first want to explain any difference in effects agnostically - i.e. just as parameters with no biological context. So maybe: ..."can be explained by the different way data is removed using each of our three missing data parameters..." THEN move on to this:
...
When using the standardised Robinson-Foulds distance to measure topological similarity, the Bayesian consensus trees recover the "correct" topology more often than Maximum Likelihood trees (Fig.~\ref{Fig_Results-global_perparam}). In fact, regardless of the amount of missing data, Bayesian consensus trees always displays the highest normalised/standardised Robinson-Foulds score (always $>$ 0.69; see \hyperref[SupplementaryMaterial]{Supplementary Material Section 3}). 
%   -Why is Baycon so different than Baytre? Since Baycon is calculated from Baytre.
We saw the poorest topological recovery using Maximum Likelihood bootstrap trees and Bayesian posterior distribution of trees (% NC: FIGURE???
). This reflects the fact it is difficult to compare two distributions of trees, and each comparison between a set of missing data parameters and "best" tree in these cases involved 1000 random pairwise comparisons rather than just one. This will obviously add noise to the results for these two methods. Also, in the case of the lower performances of the Bayesian consensus trees when using the Triplets distance, one can note the high overlap of the confidence intervals (Fig. ~\ref{Fig_Results-permeth_perparam} and Fig. ~\ref{Fig_Results-paircomp_within}-B). % NC: Don't get the relevance of the last sentence/why it fits here. Can you expand?
...
% NC: I think this can be condensed and slotted into the correct places above. So where you speak about the missing data parameters, mention differences in how they influence Rf and Triplets and why that might be. Then when you mention methods do the same.
...
%   -Go code living taxa
Using Total Evidence matrices with missing data is not a problem for recovering the correct topology as long as enough morphological data is overlapping between the fossil and living taxa. %NC: Explain more clearly what you mean by overlap - of characters?
