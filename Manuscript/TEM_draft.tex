...
When the number of living taxa with morphological data ($M_{L}$) decreases, entire rows of data are being removed from the living taxa part of the matrix. Because living taxa still have molecular characters available for phylogenetic inference (see methods), %link to methods
...
% Another practical implication of our results regards the tree building methods. Bayesian = better if chooisng one tree as backbone. However, although posterior distributions perform poorly this is more an effect of the way we compare trees (see above). Thus we do not suggest discarding posterior distribution. They are useful for comparative analyses and stuff. 
