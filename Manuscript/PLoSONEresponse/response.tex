...
%For example, the reviewer says that our data are not available when we clearly state on @@@[page and lines] that all the data are deposited in figshare @@@[link]. We suspect that had a second reviewer been found the decision might have been different. % TG: ok, this part somehow was in the SystBiol manuscript as a header part of the supplementary (1 file with three appendices) and I forgot to included somewhere in the manuscript or the supplementary in last submission (3 files). See the "OnlineAppendices" file in the GitHub folder. However, references to the GitHub page is given in the 1 supplementary material, but I assume it's a bit weak... My bad.

%We can maybe moan about the reviewer not reading carefully few things such as the comment on figure 3B "Figure 3: B is confusing. If both curves are probability distributions, then they both have to integrate to 1. So B(y) can't cover just a subset of the area covered by B(x)." where it seems he just didn't properly read the caption or about the Bhattacharya where he didn't read that we applied it because we're not comparing the means... But it's a bit weaker.

%In general I feel actually bad for some comments (i.e. typos in the equations etc...) and will remember to check more thoroughly next time.
