...
% NC: Can you make the legends the same as the ones I already fixed in the main text please???
% NC: Also most of the tables need to be in landscape
% NC: Need a short paragraph here explaining that these are supplemental results for different tree building methods or whatever, just to explain why this exists!
...
\caption{Effect of missing data on topological recovery using the Maximum Likelihood trees. The percentage of missing data (gradient colour from 0\% (white), 10\%, 25\%, 50\% and 75\% (black)) for each parameter ($M_{L}$ (upper line), $M_{F}$ (middle line) and $M_{C}$ (lower line)) is shown on both x and y axis. Topological recovery is represented by the probability of the two different tree comparison metrics (Normalised Robinson-Foulds distance (A) and Normalised Triplets distance (B)) distribution overlap with the "best" tree using the the Bhattacharyya Coefficients (low probability of overlap: blue; high probability of overlap: orange).}
...
% NC: Might need to be on landscape page or have first column narrower
\begin{table}[ht]
