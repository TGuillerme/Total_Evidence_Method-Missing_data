...
% NC: Why are the distinct topological metrics novel?
...
% NC: Worth describing the results here. 
...
% NC: Make sure this is consistent throughout, add this to where you mention the BC being between 0 and 1 too?
...
% NC: It's more likely an issue with reading figure 4. Check your figure legends. I think they might be incorrect. Then the reviewer might be seeing overlap of the wrong two distributions... But if they do overlap all the way to 25% then it is weird that the highest BC is 0.11 isn't it? TG: yes definitely. Just doubled check, it might be because the colors where miss labelled and the reviewer miss read the graph (my fault).
...
A Normalised Triplets metric value below 0 means that the placement of some taxa is worse % NC: It's worse not worth!
than expected by just randomly placing this taxon in the tree.
...
We rewrote the conclusion to deal with this comment and the specific comments 13, 52 (see comment 52 for details on the choice of citing Arcial et al 2015.) 
% NC: Is it Arcial or Arcila?
and 56.
...
This has also been observed in empirical data (e.g. Arcila et al., 2015) 
% NC: Arcial or arcila?
where Maximum Likelihood trees inferred from a Total Evidence matrix were less supported than the Bayesian consensus tree.
...
See our respone to reviewer 1's comment 11 above for changes in the revised manuscript.
