...
% NC: You can usually upload a file for the cover letter/response so pdf should be fine.
...
We gladly followed reviewer 1's graphical abstract suggestions and have changed it to be similar to Figure 5. % TG: It's freaking not catchy at all but at this point I give up. I've tried many ways but the flippin' paper IS NOT attractive. It might be useful, well written, interesting, citable, etc... But it won't be sexy.
% NC: Agreed! :)
...
% NC: Yeah I never even looked at these first time round. Mine aren't great but do fit in 85 characters!
...
"We remove data from a Total Evidence matrix by changing the values of these three parameters and then assess how this affects the topology of trees inferred % NC: Didn't one reviewer have issues with the word inferred?
...
\item{The true tree is just the starting point of the simulations:} we use the true tree only as a starting point for our simulations. Once generated, the true tree is fed to a cascade of algorithms and software (i.e. seqGen\{phyclust\}, rTraitDisc\{ape\}, RAxML, MrBayes) described in section 2.1 and 2.3 to get the best tree and the missing-data trees. Comparing the latter to the true tree will answer our question "what is the effect of our missing-data parameters and the cascade of algorithms and software on recovering the true topology?".
\end{enumerate}
...
% NC: Yes but the reviewer is asking for something different - they ask you to have no starting tree. If this is impossible in MrBayes then say so, and say you used a random tree instead. 
We found no difference in topologies between both the Bayesian inferences starting with the true tree or a random tree.
Therefore, we do not believe our results are merely the result of strong priors on the tree topology. % NC: The reviewer also says that saving time is not a good justification. It'd be worth explaining here how long it actually takes even though these datasets are small.
...
Note that a starting tree is not a Bayesian prior on topology \textit{per se} and using a fixed starting tree did not significantly % NC: Significantly as in statisically? if not change this word.
