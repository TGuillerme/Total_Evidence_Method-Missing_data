...
% NC: Line numbers? TG: I'll do all line numbering once I've incorporated all the edits to the manuscript.
...
These studies, however, were limited by because of their empirical approach limiting the conclusions of the studies to similar scenarios. %TG: or scenari? Reviewer 1 is picky with these things
Additionally, Wiens et al. (2005) didn't include fossil taxa in their analyse thus not allowing to test the placement of fossil taxa; and Manos et al. (2007) and Pattinson et al. (2014) used only patters of the fossil record to generate missing data thus not allowing to test the effect of missing data for other patterns observed for living taxa (e.g. the complete absence of morphological data; Guillerme & Cooper 2015).
% TG: Does that sound better?
Besides, Manos et al. (2007) and Pattinson et al. (2014), mainly focused on the palaeontological aspect of the question.} lines @@@.
% NC: I don't like all these i.e.s here. Why not be specific in the text rather than throwing everything into brackets? If you're only citing manos and patterson then just say what they did. TG: no more i.es then ;).
...
This is an advance on previous studies because we use large scale simulations and and analyse the effect of three distinct aspect of missing data thus focusing on both neontological and palaeontological parts of the matrix.
Besides, we test the effect of missing data by measuring two crucial aspects of topology in both Maximum Likelihood and Bayesian phylogenies: (i) the conservation of clades (based on the Robinson-Foulds distance; Robinson and Foulds, 1981) and (ii) the displacement of wild-card taxa (based on the Triplets distance; Critchlow et al., 1996) rather than just the clade conservation or support (cf. Wiens et al., 2005; Pattinson et al., 2014).} lines @@@
...
% NC: line numbers?
% \textit{We remove data from a Total Evidence matrix by changing the values of these three parameters and then assess how this affects the resulting tree topology.
...
An peculiar feature is that the Normalised Robinson-Foulds metric actually \textit{increases} when we compare the ``missing data'' trees to the ``true'' tree. % NC: specify what is being compared, e.g. "when we compare the missing data trees to the ``true'' trees."" 
...
\textit{Additionally, the Bayesian posterior trees performed more poorly than the Bayesian consensus tree (figure 4, table 1 and appendix C figure C5 and tables C5, C6 and C7).
...
\textit{The Bayesian consensus trees, however, perform badly for the Normalised Triplets metric: some parameter combinations, especially when the $M_F$ parameter reaches 75\% missing data, lead to negative values (figure 5).
...
The conclusions of theses studies, however, were limited by their empirical approach making their results applicable only to similar scenarios. %TG: or scenari
...
The conclusions of these studies were therefore restricted to these specific aspect of missing data and, besides their interesting conclusions, % TG: seems unfair to me to just bash them, they are actually nice papers
they didn't provide any insight of the combined effect of different missing data parameters.
% as well as several aspects of their experimental design (e.g. not including fossil taxa, Wiens et al., 2005; or focusing on the palaeontological aspect, Manos et al.,2007, Pattinson et al., 2014).
...
% TG: ok, actually leave it, I've tried something but the cite wasn't that convincing and we're already being carefull enough here.

% \textit{To conclude, the results of our analyses are encouraging and show that it is possible to accurately combine both neontological and palaeontological morphological data in the same phylogeny as long as there is sufficient overlap.
...
% NC: I don't like this last bit. It's really weak. I'd cut it to be honest. The first sentence is not very clear (suddenly you revert back to neo and paleo) and the second sentence has no real explanation or link to the rest of the paper. TG: let's just remove it then... You're right it's totally useless and after reading the whole paper, the reader won't mind saving a few more second of their life.
...
% NC: Do you want to say values > 0.95 are significantly similar, values < 0.05 are significantly different?  TG: yes.
...
\textit{The detailed MrBayes parameters are available in Appendix A. We also included analysis showing details on the $\alpha$ parameter estimation though it is beyond the scope of this paper and are not discussed further here.} lines 336-338 @@@
See also Appendix A "Morphological rates estimations results".

% NC: Needs a bit more explanation. Something like, we also include in the appendix an analysis showing ... though it is beyond the scope of this paper to discuss these results further here. TG: is that ok?
...
\textit{Our results show that the topology of the Bayesian consensus tree is always closer to the ``best'' tree topology than the ``best'' Maximum Likelihood tree (Figure 5).
...
\textit{Because of the difficulties in representing so many pairwise comparisons in a meaningful way, we summarized these results as a heat map of Bhattacharyya Coefficients (see Figure 6).
...
\textit{Using both Normalised Robinson-Foulds and Normalised Triplets metrics from the Bayesian consensus trees, the parameter combination with no missing data (i.e. $M_{L}$ = 0\%, $M_{F}$ = 0\%, $N_{C}$ = 100) is always the most dissimilar to all the other parameter combinations (thin deep blue line at the base of Figure 6).
...
% NC: No we discussed this last night - I think they *do* mean "best" as in our protocol. So just make the changes to the first two rows. TG: Erf, deeply sorry, I probably forgot to take the not here while chatting. Ok after reflection I either don't remember at all what I said or either don't agree. The tables show the "Summary of the comparisons between the "best" tree and the "missing-data" trees for each different tree inference method using either the Normalised Robinson-Foulds metric (RF) or the Normalised Triplets metric (Tr)." 
%
% Tree inference method       Metric   Min.  1st Qu.  etc...
% Maximum Likelihood tree       RF      1.1     2.2     3.3
%
% And this reads: the comparison between the "best" <insert what's in the Tree inference method column> tree compared to the "missing-data" trees in the same method for the <insert metric> is minimum <insert value> etc...
...
% NC: Can you rephrase a little here so it sounds less like you saying "no, and you're stupid". Doesn't the response to reviewer 1 cover this already? I've had a go but you may need to check what I've written. TG: I've added the bit at the start saying "no but look, we did it in the appendix". I didn't really get the "and you're stupid" (I'm not the best person to judge these things ;)). Maybe add something along the line "And we're sorry for the confusion, we now the manuscript is long and complicated but we made a effort of describing each step in great details, even the random seed generation..."
...
Some fossils may retain 100\% of their data and others may lose all their data at this parameter value (down to a minimum of 5\% available data; see below)
%up to a maximum of 95\% missing data, see below).
% NC: normally you'd say up to a maximum rather than up to a minimum. Is what I've written correct? TG: maybe change it "down to a minimum"?
