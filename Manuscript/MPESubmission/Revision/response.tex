...
% To do (see reviewer 1 specific comment 13)
...
Nonetheless, we would rather prefer not abbreviate these terms: in fact, even though they might add some "smoothness" % TG: that's not the word
...
We clarified and unified % TG: or standardised?
...
\end{enumerate}
Both points were discussed in the former manuscript lines 566 and 573 (or lines @@@ and @@@ in the revised version). %TG: maybe expand this bit then?
...
% TG: a a paragraph in the results about the wild-card fossils
As mentioned in the previous comment (reviewer 1 comment 9), this is due to the fact that the Bayesian consensus tree will be less and less resolved when missing data increases.
...
"We selected this model and parameters settings to be a compromise between realism from empirical datasets (e.g. Kelly et al., 2014) %\citep[e.g.][]{kellymolecular2014}
