...
% TG: Not sure if the last bit is necessary...
...
% NC: Maybe add something like this if this is your intention. It's probably worth it. No need to keep lots of un-necessary files. TG: Done and agree
...
% TG: maybe add a sentence in the manuscript?

% NC: This argument [TG: the one commented out below] makes no sense. The point the reviewer is making is that we go:
...
% TG: you're right, after reflection, I think we should have never mentioned the ``true'' tree and just say in the methods that we used a random Birth-Death tree with such and such parameters to generate the matrix. It's kind of too late to backup by now but the argument was just kind of saying: "we don't care if we can recover the "true" tree because we don't give a flip on how the algorithms perform at making a matrix". It's all random anyway...
% TG : I've commented out the non-sense justification below and wrote something shorter above (and referring to reviewer's 1 comment).
