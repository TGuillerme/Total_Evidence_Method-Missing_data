...
%%%%
We take on board the coment and agree that this would be a bad bias if it was the case. However we ran extra analysis and there is no bias.
%%%%
...
We compared the topology of these trees in Bayesian inference when using the true tree as a starting tree or not using a starting tree (i.e. using the default in MrBayes which is using a random tree).
...
% TG: Add that to the caption
% Pooled the data across the different missing data parameters
...
Homoplasy is a difficult concept, not really a value so we went for...
...
% NC: I think you misunderstood this comment so I have rephrased. Their issue was you were recommending people use constraints for tip dating, but then saying not using a constraint might make tip dating better. Instead I've made it so you're saying IF you need a constraint, use this, but be careful. TG: Deadly! Thanks!
...
% NC: Sometimes in this you have N_C, sometimes M_C. FIX IT!!! TG: Following reviewer 2's comment, it's now N_C rather than M_C. 
...
% There is no additional effect of $M_{F}$ and $N_{C}$, regardless of the amount of missing data in these parameters (Figure 6A).
...
% NC: This sentence and the one before it seem to be the same thing? TG: Yep, I modified to:
...
% TG: we really apologize but we think there's an error. We don't know where their coming from but it's probably our fault because of mislabelling. Hopefully this is fixed.

% Check again table. Difference between tables and figures.
...
% And fix this one with adding the tables.
% Marginal probabilities checking. Do it?
...
% If we understand the second point correctly, we have not added "best" to these as the Maximum Likelihood tree is by definition the tree with the highest likelihood, and the Bayesian consensus is the consensus tree of the Bayesian inference trees. So adding the word best here is not entirely accurate.

Change caption in the "best" ML tree and "best" Bay con tree to blablabla.
...
% NC: Maybe add something like this if this is your intention. It's probably worth it. No need to keep lots of un-necessary files. TG: Agree!
...
% NC: Ah OK. You haven't corrected this everywhere in these comments I don't think... TG: Should be good now.
...
% TG: I've added the following to the new removing data section:
