...
%Packages
\setlength\parindent{0pt} %NC: To get rid of automatic indenting
...
% NC: You can usually upload a file for the cover letter/response so pdf should be fine. TG: Cool!
...
We rephrased the highlights as follows: %3 to 5 bullet points (maximum 85 characters, including spaces, per bullet point). TG: probably need improvement here.
% NC: Yeah I never even looked at these first time round. Mine aren't great but do fit in 85 characters! TG: that looks way better, thanks!
...
% NC: Don't forget to change stuff in the manuscript if I change it here and vice versa.
...
% NC: Argh this is really confusing. It doesn't logically flow
...
% For morphological and molecular data separately, both studies show that missing data is not a major problem and should not be an obstacle to combine both living and fossil species in the same phylogenies.
% However, they are limited by their empirical-only approach (Wiens et al., 2005) and the scheme of generating missing data (Manos et al., 2007; Pattinson et al., 2014).
% Also both studies focus on the palaeontological aspect of the question (i.e. the fossil data)." lines 85-98 @@@
%%% NC: Paragraph above needs work aka a bit of logic to it.

% TG: how's that (+idea flow):
...
"In this study, we propose a theoretical assessment of the effect of missing data in the Total Evidence method by removing living or fossil data, or all data for certain characters. % NC: All data overall suggests an empty matrix
...
% "We remove data from a Total Evidence matrix by changing the values of these three parameters and then assess how this affects the topology of trees inferred % NC: Didn't one reviewer have issues with the word inferred? TG: for the matrices only (and he/she's right); trees are still inferred.
% using Maximum Likelihood and Bayesian methods by using two different topological distance metrics, based on the Robinson-Foulds (Robinson and Foulds, 1981) and Triplets (Critchlow et al., 1996) distances.
% Thus, in the present study, we propose a new approach to the missing-data question, focusing on both palaeontological and neontological data (rather than just palaeontological data), using only simulated data and two distinct topological distance metrics." lines 127-134 @@@
% NC: Why are the distinct topological metrics novel? TG: not novel but novel to use two aspects (people used only clade related metrics before).
...
% NC: Worth describing the results here. TG: Done.
...
% "In practice, the difference between the ``true'' and the ``best'' tree represents the effect of our parameters and R packages choices %NC: What dod you mean by R packages choices? Do you mean the way the trees were simulated?
%  (i.e. diversitree, phyclust and ape) as well as the phylogenetic methods used (see appendix B).
...
The difference between the ``true'' and the ``missing data'' tree, however, represents the effect of our parameters used to generate the ``true'' tree and the algorithms used to generated the ``complete'' matrix as well as the effect of our missing data parameters and the phylogenetic methods used.
Because the main aim of this study, is to look at the effect the missing data parameters on topological recovery, we chose to represent only the comparisons between the ``best'' trees to ``missing data'' trees.
The results of the comparisons of the ``true'' and the ``missing data'' tree are available in appendix B.
Note that these results makes little difference to our overall results." line 276-280 @@@.

See also appendix B section "Differences between the ``true'' and the ``best'' trees."
% NC: Needs a little more explanation, and a line about what analysis you did in appendix b. So something along the lines of "We compare best trees to missing data trees, but could also compare true trees to the missing data trees. We do not do this because... However, results of analyses comparing the two can be found in appendix B. This makes little difference to our overall results." That might need to be better integrated with the paragraph(s) before.
...
We performed an extra analysis on a subset of five trees with the three missing data parameters at 0\%, 10\%, 25\%, 50\% and 75\% for 20 chains. 
...
% NC: Yes but the reviewer is asking for something different - they ask you to have no starting tree. If this is impossible in MrBayes then say so, and say you used a random tree instead. TG: Maybe there's a phylogeny confusion here. In both ML or Bayesian, you need to start from a tree, then calculate the likelihood of all the parameters, then change the tree, calculate again, etc...
We found no difference in topologies between both the Bayesian inferences starting with the true tree or a random tree.
Therefore, we do not believe our results are merely the result of using a specific starting tree topology. %strong priors on the tree topology. TG: That's the big difference that the PLoS ONE reviewer also didn't got. A starting tree is not a Bayesian prior (in P(A|B) = P(B|A)*P(A)/P(B; P(A) is the prior). It's just a user fixed starting point for the MCMC walk rather than a random one.
% NC: The reviewer also says that saving time is not a good justification. It'd be worth explaining here how long it actually takes even though these datasets are small.
%
% TO DISCUSS @@@
% TG: about that: it using a random tree or the true tree doesn't change the timing significantly. The whole idea was suggested by Rob or Gavin (don't remember exactly) at lunch time when I was visiting Gavin in Sheffield. To speed up the search in the tree space they suggested I already start from a sensible tree in the tree space, that was meant to speed up the analysis.
%
These results are available in the supplementary materials Appendix A (figure A2 and tables A1 and A2) and we also added a note to the manuscript:
...
Note that a starting tree is not a Bayesian prior on topology \textit{per se} and using a fixed starting tree did not significantly % NC: Significantly as in statisically? if not change this word. TG: yap, p-value all all the mumbo-jumbo
affect topology (see Appendix A, section ``Effect of the starting tree on Bayesian inference'')." line 323-327 @@@.
...
Values in between these two thresholds show the probability of overlap between the distributions but do not allow us to define the significance of the similarity or differences between distributions." line 419-424 @@@.

% NC: Make sure this is consistent throughout, add this to where you mention the BC being between 0 and 1 too? TG: Done.
...
% NC: It's more likely an issue with reading figure 4. Check your figure legends. I think they might be incorrect. Then the reviewer might be seeing overlap of the wrong two distributions... But if they do overlap all the way to 25% then it is weird that the highest BC is 0.11 isn't it?
% TG: yes definitely. Just doubled and tripled checked, it might be because the colors where miss labelled and the reviewer miss read the graph (my fault). In the tables 1, C5, C6 and C7, RF overlap are always low (max < 0.11) and Tr overlap are always high (max ~80+). And that's what's visible in figure 4. Grey and orange lines do not overlap in RF and do overlap in Tr.
...
% NC: Is it Arcial or Arcila? TG: Dahiana Arcila

% "Previous studies have explored the effect of missing morphological data when combining living and fossil species in phylogenies but where limited by their empirical approach or their aspects of missing data (Wiens et al 2005; Pattinson et al 2014). % NC: I still don't get these limitations. How are they are issue? What aspects?
% TG: Limitations are:
% Empirical data (Wiens et al., 2005; Manos et al.,2007, Pattinson et al., 2014)
% No fossils (Wiens et al., 2005)
% Scheme of generating missing data (Manos et al.,2007, Pattinson et al., 2014)
% Focus on palaeontological aspect of missing data (Manos et al.,2007, Pattinson et al., 2014)
% Using clade conservation metrics only (Wiens et al., 2005; Pattinson et al., 2014)
...
We apologise for this silly mistake and fixed the colors in the figure. We also changed the color scheme to match with the general color scheme of the paper (black, grey, orange and blue).
...
% NC: The reviewer means that you're presenting results from across all your different parameters, and all your different amounts of missing data in the table. You need to clarify this, especially as in figures you separate these out. Pooling just means combining. I noticed this on my re-read just now too.
...
% NC: If it's just you doing exactly what they ask for there is no need to re-write the sentence.
...
% NC: Hmmm I reckon this is probably inconsistent throughout then as I usually correct it to plural. I'd just change this tbh. Or at least check you've been consistent. TG: ok maybe be I have a plural/singular problem (I definitely have). I actually never know when to use it a singular or plural: "One data point", that's ok, but "morphological data for one taxa"? Would that be singular or plural?
...
(2) - the second point underlined by the reviewer in this specific comment strikes use as really interesting. However, we considered this study to already be long and complex. We therefore think that adding an extra question to this study (i.e. what is the effect of missing data on the morphological substitution rate parameter estimates?) might lose the reader and make an already long paper even longer.
Nonetheless, we added the result of these analyses to \textbf{Appendix-A Morphological rates estimations results}.
...
% NC: I'm a bit confused - did you do what they wanted or not? If not then the whole "this will make the paper longer" argument holds. If you did do what they wanted then it might need a little more explanation somewhere. You can't just throw random stuff into the appendices for no reason.
% TO DISCUSS @@@
% TG: Ok, maybe we should remove it. I've done the analysis because it's interesting and easy to do but maybe we can just add the results to this response, not to the revised manuscript?
...
% NC: Check this - I'm sure I caught one up above in the comments here. TG: I think the idea is that we didn't "infer" any matrices, we simulated/generated/created them. The trees are still inferred though.
...
% NC: I think what is needed here is a definition of what is "too much". Is it 50%? 20%? etc. Too much is arbitrary.
% TO DISCUSS @@@
% TG: That's why I removed the "too much". I'm actually not sure how Homoplasy is measured (as in units of homoplasy). Wagner did a lot of work on it but he usually refers to homoplasy through proxies like characters retention, etc...
...
% "We selected this model and these parameters to strike a balance between realism for empirical datasets (e.g. Kelly et al., 2014) and models with few parameters (cf. Springer et. al., 2012) to reduce the computational time required if these parameters were estimated rather than defined in the tree building part of the analysis (even with the parameters defined, the total computational time for the whole analysis was around 150 CPU years)." lines 192-198 @@@
...
% NC: Why not just use the phrasing given by the reviewer for the first part? TG: Done.
...
% NC: Might need a bit more explanation. For example mentioning that "Topological error, however, becomes minimal for all scenarios (Parsimony and Mk, with or without missing data) at a rate of 0.5 (see figure 3 from Wright and Hillis (2014)" as you've mentioned above.
% TG: added a sentence.
...
% NC: No, only thank them if it's something really stupid - like the mislabeling of the figure axes - or really insightful.
...
Using the Bayesian consensus tree rather than the Maximum Likelihood can also reduce the number of false positive topologies.
...
% NC: I think you misunderstood this comment so I have rephrased. Their issue was you were recommending people use constraints for tip dating, but then saying not using a constraint might make tip dating better. Instead I've made it so you're saying IF you need a constraint, use this, but be careful.
...
%NC: Ummm but specific comment 50 refers to comment 11. Should this also refer to comment 11?
...
We removed the sentence about the t-test similarity from both figure captions. %TG: not sure if that's what the reviewer wanted. % NC: Yeah I think that's what they meant
...
% NC: Sometimes in this you have N_C, sometimes M_C. FIX IT!!!

% TO DO: thorough check through. @@@
...
There is no additional effect of $M_{F}$ and $M_{C}$, regardless of the amount of missing data in these parameters (Figure 6A).
...
% NC: This sentence and the one before it seem to be the same thing?
...
% NC: The "wording is off" means the wording is incorrect. Probably means the second sentence not paragraph. I've editted the caption as mentioned further up in the comments.

We fixed the highlighting in bold to the classical values of 0.05 or 0.95.
...
% NC: OK I think the point here is that Table 1 has a column saying the "Max." BC score. So the reviewer is saying that this is happening:
% In Appendix C, B posterior vs B consensus for some parameter value is 0.5 (for example), but the max value when you compare this in Table 1 is 0.11. How is this possible???????
% NC: So your response here is completely irrelevant.
...
% NC: https://en.wikipedia.org/wiki/Marginal_distribution
% NC: Marginal probabilities are basically the probability of one thing given you're controlling for another thing. So they are looking to see the effect of tree inference method independent of parameter differences. Otherwise they suggest you are hiding important differences via this summarizing over all parameter combinations. 
% NC: I think you can argue that yes, table 1 hides some important effects, but that it was difficult to summarize these results and more in depth tables can be seen in the appendix.
...
We removed the repetition in the caption. If we understand the second point correctly, we have not added "best" to these as the Maximum Likelihood tree is by definition the tree with the highest likelihood, and the Bayesian consensus is the consensus tree of the Bayesian inference trees. So adding the word best here is not entirely accurate.

%Not sure I understand the second part of the comment. % NC: I think they are a bit confused and think you need to explain it is the best ML and best bayesian tree. But that's what ML and consensus means. Maybe they want clarification that it is only one tree in each of those and multiple trees in the others? I've added a bit above.
...
% NC: Maybe add something like this if this is your intention. It's probably worth it. No need to keep lots of un-necessary files.
We will also clean up this repo once the paper has been accepted. At present it is functioning both as our working repo for code as well as the repo for this paper.
...
We understand the concerns of the reviewer but, again, we do not agree that comparing the "missing-data" trees to the "best" trees adds a layer of complexity to our simulations.
...
The problem we are tackling in this manuscript is to understand "what is the effect of our three parameters on recovering topology ?"
To answer this question, we have to add a layer between the data (i.e. the "complete" or "missing-data" matrices) and the actual topology which is the tree inference.
We test the effect of this layer by using different tree inference methods.

% NC: Note: problematic does not mean what you think it does... Something can be problematic (it's an adjective not a noun).

Because we use a simulation approach to test this problem, we have to start from a random starting point (the "true" tree) and then do the following steps: (1) generate and remove the data (2) infer the topologies and (3) compare the topologies.
Note that in this schematic representation, the "true" tree is the step (0) of our analysis and involves a series of algorithms and software (i.e. seqGen\{phyclust\}, rTraitDisc\{ape\}, RAxML, MrBayes).
To compare the "true" tree's topology to the "missing-data" topologies will be equivalent of exploring the effect of our three parameters on recovering topology through 3 layers (1- generate the data; 2- infer the topologies; and 3- compare the topologies).
...
% NC: This argument makes no sense. The point the reviewer is making is that we go:
% True tree > Best tree > comparisons when we could just go True tree > comparisons. So our approach *is* more complex than their suggestion. Instead the justification here is the same as used for reviewer one - that in real life scenarios we never know the true tree and we want it to be as close to real life as possible. BUT that we have tested their idea and find it makes no difference but all the results are in the appendix.

However, we have added analyses with comparisons between the "true" tree and the "missing-data" trees in the Appendix C (Figures C6 and C7).
It is interesting to note that the algorithms and software as well as probably the size of our matrices are actually not ideal for the recovery of the "true" tree is rather low and scales down the our results (see figure @).% NC: HUH????
However, even though these comparisons are worse than comparing the best tree to the missing data trees, it is encouraging to see that the patterns in the results are conserved (i.e. Bayesian consensus trees $>$ Maximum Likelihood trees $>$ both bootstraps trees and Bayesian posterior trees).
...
"To explore the effect of missing morphological data on topological recovery, we removed various amounts of the “complete” matrix to obtain matrices with missing morphological data. Hereafter, we call these matrices with missing morphological data the “missing-data” matrices.
% NC: add clarification here about molecular data
Note that the amount of molecular data remained constant throughout our simulations - XX in living taxa and no molecular data for fossil taxa (see above).
% NC: Is this correct?
...
% NC: Ah OK. You haven't corrected this everywhere in these comments I don't think...
...
% NC: OK - I'd add a line somewhere in the manuscript to make this clear. something like:

%"Note that 55\% missing data for fossil taxa does not mean that each fossil is missing 50\% of it's morphological data. Instead this 50\% refers to the whole matrix. Some fossils may reatin 100% of their data and others may lose all their data at this parameter value"

Also, a second argument for using this slightly simplistic way to deal with the missing data in the fossil record follows our main recommendation from this study: the best way to improve the quality of Total Evidence trees is to have a maximum number of living species with available morphological data.
...
% NC: Not sure the second paragraph is needed. The reviewer is saying that they want to know what happens if living taxa have all their data but fossils have variable amounts. And I think this is what you've done for some parameter combinations. Just need to apologise for not being clear and clarify in the text how data were missing from fossils. Doesn't need so much waffle and defensiveness - they are asking you to do what you've already done! :)
