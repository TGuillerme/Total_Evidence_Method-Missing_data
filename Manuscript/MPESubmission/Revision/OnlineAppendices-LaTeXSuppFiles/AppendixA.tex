...
\begin{table}[ht]
\caption{Effect of the using the ``true'' tree as a starting tree on topological recovery for the two Normalised Robins-Foulds (RF) and Triplets (Tr) metrics using simple linear regression.} % NC: I'm not sure what these two tables are telling you. Clarify. The caption says nothing about the random starting trees.
\label{Tab_lm_res}
\centering
