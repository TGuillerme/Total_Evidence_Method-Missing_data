...
\begin{flushleft}
{\Large
\textbf\newline{Effects of missing data on topological inference using a Total Evidence approach}
}
\newline
...
The "best" tree and the "missing-data" trees were inferred using RAxML v. 8.0.20 \cite{Stamatakis21012014}. For the molecular data, we used the GTR + $\Gamma_4$ model \cite{tavare1986} (default GTRGAMMA in RAxML v. 8.0.20  \cite{Stamatakis21012014}). For the morphological data, we used the implemented Markov \textit{k} state model \cite{lewisa2001} assuming an equal state frequency and a unique overall substitution rate ($\mu$) following a gamma distribution of the rate variation with four distinct categories (M\textit{k} + $\Gamma_4$; -K MK option in RAxML v. 8.0.20 \cite{Stamatakis21012014}).
...
The Robinson-Foulds distance \cite{RF1981}, or "path difference", measures the number of shared clades across two trees. The metric reflects the distance between the distributions of tips among clades in the two trees \cite{RF1981} (see Appendix S2 for calculation details). This metric is bounded between one, when the two trees are identical, and $n-2$ (for two trees with $n$ taxa) when there is no shared clade in the two trees. This metric is sensitive to minor changes in clade conservation: if the trees are composed of two clades of three taxa (\textit{(((a,b),c),((d,e),f))}), the swapping of any two taxa will lead to a maximal score of the Robinson-Foulds distance indicating poor tree similarity. We normalised this metric by dividing the Robinson-Foulds distance by the mean Robinson-Foulds distance from 1000 random tree comparisons \cite{Bogdanowicz2012}. This allows us to compare the metric for any number of taxa by scaling it with the expected value of the metric when comparing two random trees (see Appendix S2 for the calculation details). Therefore, when the Normalised Robinson-Foulds distance between two trees is equal to one, the trees are identical; if the distance is equal to zero, the trees are no more different than expected by chance; finally if the distance is less than zero, the trees are more different than expected by chance.

\paragraph*{Triplets distance}
The Triplets distance \cite{dobson1975triplets} measures the number of sub-trees made up of three taxa that differ between two trees \cite{critchlowthe1996} (see Appendix S2 for calculation details). This metric measures the position of each taxon and clade in relation to its closest neighbours. It is bounded between zero when the two trees are identical and $\binom{n}{4}$ (for two trees with $n$ taxa) when there is no shared taxa/clade position in the two trees. Therefore this metric sensitive to the conservation of wildcard taxa. We normalised this metric in the same way as for the Robinson-Foulds distance resulting in the Normalised Triplets distance.
...
The Normalised Robinson-Foulds distance is always lower for the Maximum Likelihood trees than for the Bayesian consensus trees (median Bhattacharrya Coefficient = 0.69, 0.48 and 0.66 for $M_{L}$, $M_{F}$ and $M_{C}$ respectively; Fig.~\ref{Fig_Results-permeth_perparam}; Tables S5, S6 and S7). 
...
\subsection*{Supplementary informations}
\label{S1_Appendix1}
\noindent{\bf Appendix S1 Tree Inference.} Supplementary information on the Tree Inference methods.

\label{Figure S1}
\noindent{\bf Figure S1 proportion of morphological characters states.}

\label{S2_Appendix2}
\noindent{\bf Appendix S2 Tree Comparisons.} Supplementary information on the Tree Comparison methods.

\label{S2_Appendix3}
\noindent{\bf Appendix S3 Additional results.}.

\label{Figure S2}
\noindent{\bf Figure S2 Effect of increasing missing data on topological recovery using Maximum Likelihood Bootstrap trees (black) and Bayesian posterior tree distribution (grey).}

\label{Figure S3}
\noindent{\bf Figure S3 Effects of missing data on topological recovery using Maximum Likelihood trees.} 

\label{Figure S4}
\noindent{\bf Figure S4 Effects of missing data on topological recovery using Maximum Likelihood bootstrap trees.}
 
\label{Figure S5}
\noindent{\bf Figure S5 Effects of missing data on topological recovery using Bayesian posterior tree distribution.}

\label{Table S1}
\noindent{\bf Table S1 Comparisons between the "best" tree and the "missing data" trees for each different tree inference method.}

\label{Table S2}
\noindent{\bf Table S2 Comparisons between the "best" tree and the "missing data" trees for each different tree inference method for the $M_{L}$ parameter.} 

\label{Table S3}
\noindent{\bf Table S3 Comparisons between the "best" tree and the "missing data" trees for each different tree inference method for the $M_{F}$ parameter.} 

\label{Table S4}
\noindent{\bf Table S4 Comparisons between the "best" tree and the "missing data" trees for each different tree inference method for the $M_{C}$ parameter.} 

\label{Table S5}
\noindent{\bf Table S5 Bhattacharyya Coefficients of the pairwise method comparisons for the $M_{L}$ parameter.} 

\label{Table S6}
\noindent{\bf Table S6 Bhattacharyya Coefficients of the pairwise method comparisons for the $M_{F}$ parameter..} Bhattacharyya 

\label{Table S7}
\noindent{\bf Table S7 Bhattacharyya Coefficients of the pairwise method comparisons for the $M_{C}$ parameter..} Bhattacharyya 
...
Conceived and designed the experiments: TG NC. Ran the analysis: TG. Analyzed the data: TG. Wrote the paper: TG NC.
